...
export const SpamController ={
    async markSpam(req, res) {
        try {
            ...
            let spamRecord = await Spam.findOne({ where: { phoneNumber } });

            if (!spamRecord) {
            // If the spam number does not exist, create a new record
            spamRecord = await Spam.create({ phoneNumber });
            }
            ...
            if (!userAlreadyAssociated) {
                ...
                await spamRecord.update({ spamReport: currentSpamReport });
